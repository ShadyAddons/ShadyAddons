...
public class AutoArrowAlign {
    ...
    public void onTick(TickEndEvent event) {
        ...
        if (objectMouseOver != null && objectMouseOver.entityHit != null) {
            ...
            if (toClickMap.containsKey(BP)) {
                if (toClickMap.get(BP) == 0) return;
                if (thread == null || !thread.isAlive()) {
                    thread = new Thread(() -> {
                        try {
                            for(int i = 0; i < toClickMap.get(BP); i++) {
                                rightClick();
                                Thread.sleep(Config.autoArrowAlignDelay);
                            }
                            Thread.sleep(200);
                            trackClicks.remove(BP);
                            accurate = false;
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
    ...
    public void onRender(RenderWorldLastEvent event) {
        ...
        if (!start) {
            ...
            if (objectMouseOver != null && objectMouseOver.entityHit != null) {
                Entity entity = objectMouseOver.entityHit;
                if (entity instanceof EntityItemFrame && isInSection3()) {
                    start = true;
                }
            }
        }
    ...
    private static void initFrame() {
        ...
        Utils.sendModMessage("Pattern Found: " + pattern);
        switch (pattern) {
            case "legs":
